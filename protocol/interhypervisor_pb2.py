# Generated by the protocol buffer compiler.  DO NOT EDIT!
# source: interhypervisor.proto

import sys
_b=sys.version_info[0]<3 and (lambda x:x) or (lambda x:x.encode('latin1'))
from google.protobuf import descriptor as _descriptor
from google.protobuf import message as _message
from google.protobuf import reflection as _reflection
from google.protobuf import symbol_database as _symbol_database
from google.protobuf import descriptor_pb2
# @@protoc_insertion_point(imports)

_sym_db = _symbol_database.Default()




DESCRIPTOR = _descriptor.FileDescriptor(
  name='interhypervisor.proto',
  package='',
  syntax='proto2',
  serialized_pb=_b('\n\x15interhypervisor.proto\",\n\x0b\x41pplication\x12\x0c\n\x04name\x18\x01 \x02(\t\x12\x0f\n\x07package\x18\x02 \x02(\x0c\"\x16\n\x08PingPong\x12\n\n\x02id\x18\x01 \x02(\x05')
)
_sym_db.RegisterFileDescriptor(DESCRIPTOR)




_APPLICATION = _descriptor.Descriptor(
  name='Application',
  full_name='Application',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='name', full_name='Application.name', index=0,
      number=1, type=9, cpp_type=9, label=2,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
    _descriptor.FieldDescriptor(
      name='package', full_name='Application.package', index=1,
      number=2, type=12, cpp_type=9, label=2,
      has_default_value=False, default_value=_b(""),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  options=None,
  is_extendable=False,
  syntax='proto2',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=25,
  serialized_end=69,
)


_PINGPONG = _descriptor.Descriptor(
  name='PingPong',
  full_name='PingPong',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='id', full_name='PingPong.id', index=0,
      number=1, type=5, cpp_type=1, label=2,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  options=None,
  is_extendable=False,
  syntax='proto2',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=71,
  serialized_end=93,
)

DESCRIPTOR.message_types_by_name['Application'] = _APPLICATION
DESCRIPTOR.message_types_by_name['PingPong'] = _PINGPONG

Application = _reflection.GeneratedProtocolMessageType('Application', (_message.Message,), dict(
  DESCRIPTOR = _APPLICATION,
  __module__ = 'interhypervisor_pb2'
  # @@protoc_insertion_point(class_scope:Application)
  ))
_sym_db.RegisterMessage(Application)

PingPong = _reflection.GeneratedProtocolMessageType('PingPong', (_message.Message,), dict(
  DESCRIPTOR = _PINGPONG,
  __module__ = 'interhypervisor_pb2'
  # @@protoc_insertion_point(class_scope:PingPong)
  ))
_sym_db.RegisterMessage(PingPong)


# @@protoc_insertion_point(module_scope)
